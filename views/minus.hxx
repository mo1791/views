...
namespace details
{
    ...
    concept Substractable = requires( LHS lhs, RHS rhs ) { lhs / rhs; };   
} // namespace details



// Custom view adaptor that substract a specified value from each element of a range.
...
public:
    ...
    {
        return std::invoke(std::minus{}, *m_cursor, m_parent->m_value);
    }
...
public:
    template <typename range_t>
        requires std::conjunction<
                    ...
                    std::bool_constant<
                        details::Substractable< T, std::ranges::range_value_t<range_t>>>
                >::value
...
namespace views {
    inline constexpr divides_view_adaptor minus{};
}
