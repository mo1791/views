...
namespace details
{
    ...
    concept Multiple = requires( LHS lhs, RHS rhs ) { lhs * rhs; };    
} // namespace details



// Custom view adaptor that multiplu a specified value to each element of a range.
...
public:
    ...
    {
        return std::invoke(std::multiplies{}, *m_cursor, m_parent->m_value);
    }
...
public:
    template <typename range_t>
        requires std::conjunction<
                    ...
                    std::bool_constant<
                        details::Multiple< T, std::ranges::range_value_t<range_t>>>
                >::value
...
namespace views {
    inline constexpr divides_view_adaptor multiplies{};
}
