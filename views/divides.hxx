...
namespace details
{
    ...
    concept Divisible = requires(LHS lhs, RHS rhs) { lhs / rhs; };
}


// Custom view adaptor that divides each element of a range by a specified
// value.
...
public:
    ...
    {
        return std::invoke(std::divides{}, *m_cursor, m_parent->m_value);
    }
...
public:
    template <typename range_t>
        requires std::conjunction<
                    ...
                    std::bool_constant<
                        details::Divisible< T, std::ranges::range_value_t<range_t>>>
                >::value
...
namespace views {
    inline constexpr divides_view_adaptor divides{};
}
