...
namespace details
{
    ...
    concept Modulable = requires( LHS lhs, RHS rhs ) { lhs % rhs; };    
} // namespace details



// Custom view adaptor that performe modulus ops to each element of a range.
...
public:
    ...
    {
        return std::invoke(std::modulus{}, *m_cursor, m_parent->m_value);
    }
...
public:
    template <typename range_t>
        requires std::conjunction<
                    ...
                    std::bool_constant<
                        details::Modulable< T, std::ranges::range_value_t<range_t>>>
                >::value
...
namespace views {
    inline constexpr divides_view_adaptor modulus{};
}